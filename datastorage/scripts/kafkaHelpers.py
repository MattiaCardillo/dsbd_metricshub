    ...
    try:
        while (True):
            ...
            for msg in metricMsg["metrics1hData"]:
                dbHelpers.makeQuery(dbHelpers.queries["insertMetric1h"], (msg["metric"], msg["max"], msg["min"], msg["avg"]))
            
            for msg in metricMsg["metrics3hData"]:
                dbHelpers.makeQuery(dbHelpers.queries["insertMetric3h"], (msg["metric"], msg["max"], msg["min"], msg["avg"]))
            
            for msg in metricMsg["metrics12hData"]:
                dbHelpers.makeQuery(dbHelpers.queries["insertMetric12h"], (msg["metric"], msg["max"], msg["min"], msg["avg"]))
        c.close()
